...
x_pix_data = [i[0] for i in pixel_values]
x_angle_data = [i[0] for i in servo_angles]
size_pix = np.shape(x_pix_data)
if(size_pix[0]%2!=0):
        x_pix_data = np.delete(x_pix_data,-1)
        x_angle_data = np.delete(x_angle_data,-1)
size_pix = np.shape(x_pix_data)
coeffs = np.polyfit(x_pix_data, x_angle_data, size_pix[0]-1)
print(coeffs)
print(x_pix_data)
print(x_angle_data)
angle = np.polyval(coeffs,500)
time.sleep(4)
s.setTarget(R_hor,int(angle))
time.sleep(6)
ret, img = cap.read()
warped_img = getWarp(img, M)
cv.imshow('image ',warped_img)
